...
- [How to restore filesystems from an archive](#how-to-restore-filesystems-from-an-archive)
- [How to backup and restore directories](#how-to-backup-and-restore-directories)

## How to configure fsarchiver.tcl

Configuration is done during [Installation](README.md#installation) and a reinstall should not cause any issues. 

But, to tweak something like colors without doing a full install

- Colors of fsarchiver-helpers are configurable:
	- e.g. changing the suggested skin color of mc (seasons-autumn16M)
		* To something like sand256 (light) or ristretto256 (dark)
	- The default dialog colors can be changed by modifying or deleting the autumndc.rc file in configdir. configdir is typically ~/.config/fsarchiver-helpers but could be customized, enter the following to display the current setting:
...
> [!IMPORTANT] 
> However, the reason for the default sharp colors is to remind the user (of fsarchiver-helpers) that credentials _have been elevated_
...
> [!TIP]
> for users who are not entirely familiar with mc, numbers next to menu items or buttons correspond to numbered function keys, you can use a corresponding keypress for most functions in mc (mc provides visual cues for most items i.e. which key to press). Even fsarchiver-helpers menu items could be assigned a keypress (but this is optional and left for customization)
...
+ Select an achive e.g. backup-msata-chrisrobin-trixie-20250621.fsa
+ keypress F2
+ Inspect the archive via [fsarchiver archinfo](#how-to-display-info-about-an-fsarchiver-archive)
...
![](images/mc_fshelp_ids_to_partitions_dialog.png)
  ...
  - In this _example_ the goal is to restore the third and forth partitions of /dev/sdk with the archive filesystems ids 1 and 2 respectively.
